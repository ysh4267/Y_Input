...
internal static class MinimapDetector
{
    ...
    public static bool TryFindPlayerDot(Bitmap frame, Rectangle minimapRect, out Point dot,
                                        int minR = 200, int minG = 180, int maxB = 120)
    {
        ...
        dot = new Point((int)(sumX / count) + (area.X - minimapRect.X),
                        (int)(sumY / count) + (area.Y - minimapRect.Y));
        return true;
    }
}
